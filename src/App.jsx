...
function App() {
  ...
  const handleRegisterSubmit = (e) => {
    ...
    if (isAdminLogin) {
      const admin = loginAdmin(selectedAvatar, adminPasswordInput);
      if (admin) {
  ...
  const handleSendMessageSubmit = (e) => {
    ...
    sendMessage(messageText.trim(), attachedImage, replyMeta);
    setMessageText('');
    setAttachedImage(null);
    setImageFileName('');
    setReplyTo(null);
  ...
  return (
    <div className="dashboard-container">
      ...
      <aside className="sidebar">
        ...
        <span className="sidebar-section-title">Active Users ({users.filter(u => u.isOnline && u.id !== currentUser.id).length})</span>
                    ...
                    <div className="item-left-content">
                      ...
                      <div className="item-info">
                        <span className="item-title" style={{ fontSize: '13px', display: 'flex', alignItems: 'center' }}>
                          {u.username}
                          {u.isAdmin && <span className="admin-badge" style={{ fontSize: '8px', padding: '1px 3px' }}>Admin</span>}
                        </span>
                        <span className="item-subtitle" style={{ fontSize: '10px' }}>
                          {u.isOnline ? 'Online' : 'Offline'} {isUserInActiveRoom ? '• In Room' : ''}
                        </span>
                      </div>
                    </div>
